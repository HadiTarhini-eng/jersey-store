...
import stats from '../../data/admin/dashboard-stats.json';
...
export function AdminDashboard() {
  const { kpis, salesByDay, revenueByMonth, topCategories, topProducts, trafficSources, recentActivity } = data;

  return (
    <div className="space-y-6">
      {/* ── KPI row ────────────────────────────────────────────────────────── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <KpiCard label="Revenue"      value={kpis.revenue.value}     delta={kpis.revenue.deltaPct}     currency={kpis.revenue.currency} />
        <KpiCard label="Orders"       value={kpis.orders.value}      delta={kpis.orders.deltaPct} />
        <KpiCard label="Customers"    value={kpis.customers.value}   delta={kpis.customers.deltaPct} />
        <KpiCard label="Conversion"   value={kpis.conversion.value}  delta={kpis.conversion.deltaPct}  unit={kpis.conversion.unit} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Panel title="Top Categories" subtitle="Share of units sold">
